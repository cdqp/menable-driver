...
/**
 * Creates a UIQ timestamp from the `PerformanceCounter`.
 * The most significant bits are stored in `data[0]`.
 */
void men_get_uiq_timestamp(uiq_timestamp* out_timestamp) {
    ...
    for (int i = 0; i < num_parts; ++i) {
        const uint8_t bits_to = num_bits_in_counter - 1 - (i * UIQ_NUM_PAYLOAD_BITS);
